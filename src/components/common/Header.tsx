...
export const Header: React.FC<Props> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-xs relative">
      {/* Top Calm Sage Teal Accent Line */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-teal-600 via-emerald-500 to-teal-700" />
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <img
              ...
              className="w-11 h-11 object-contain rounded-full shadow-xs bg-white p-0.5 border border-emerald-200 shrink-0"
            />
            <div>
              <div className="flex items-center gap-2">
                <span className="text-base sm:text-lg font-black tracking-tight text-slate-900">
                  The English <span className="text-teal-700 font-black">Sekolah Alam Al-Karim</span>
                </span>
                <span className="hidden lg:inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200/80">
                  TK • SD • SMP • SMA
                </span>
              </div>
        ...
        <div className="flex items-center gap-3">
          {currentUser ? (
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Tombol Login Admin / Guru dengan Calm Sage Teal Theme */}
              <button
                id="header-admin-login-btn"
                type="button"
                onClick={onOpenLogin}
                className="flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-black text-white bg-gradient-to-r from-teal-700 via-emerald-600 to-teal-800 hover:from-teal-800 hover:to-emerald-700 border border-emerald-500/50 shadow-xs transition-all duration-150 cursor-pointer group"
